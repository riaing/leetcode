...
DP: 
"the paths from the current node to the target node consist of all the paths starting from each neighbor of the current node."
*/
class Solution {
    ...
    private void helper(int[][] graph, int index, List<Integer> cur, List<List<Integer>> res) {
        // if (graph[index].length == 0) { // 可能有的点就是没有 children！ 所以这个判断不对
        if (index == graph.length - 1) {    
            res.add(new ArrayList<Integer>(cur));
            return;
        }
        int[] nextNodes = graph[index];
        for (int n : nextNodes) {
            cur.add(n);
